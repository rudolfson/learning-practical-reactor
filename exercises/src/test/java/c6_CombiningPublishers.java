...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().flatMap(mono -> mono);
    ...
    public void task_executor_again() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().concatMap(m -> m);
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary()
                .switchOnFirst((s, f) -> {
                    if (s.get().metaData.equals("spam")) return mailBoxSecondary();
                    else return f;
                });
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().concatMap(m -> m.flatMap(id -> commitTask(id).thenReturn(id)));
